...
public class MinioBucket {
	...
	private MinioClient initMinioClient() {
		try {
			return MinioClient.builder().endpoint(new URL(this.endpoint)).credentials(this.accessKey, this.secretKey)
					.build();
		}
		catch (Exception e) {
	...
	public void upload(InputStream file, String path, String fileName, String contentType) {
		try {
			...
			minioClient.putObject(PutObjectArgs.builder().object(fileName).bucket(this.bucket).contentType(contentType)
					.stream(file, -1, 600000000).build());
		}
		catch (Exception e) {
	...
	public String retrieve(String path) {
		try {
			MinioClient minioClient = this.initMinioClient();
			minioClient.getObject(GetObjectArgs.builder().object(path).bucket(this.bucket).build());
		}
		catch (Exception e) {
			throw new RuntimeException(e.getMessage());
		}
	...
	public String toString() {
		return "MinioBucket{" + "endpoint='" + endpoint + '\'' + ", accessKey='" + accessKey + '\'' + ", secretKey='"
