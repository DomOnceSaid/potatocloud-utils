...
//            FileOutputStream outputStream = new FileOutputStream("src/main/resources/"+ timeStamp + ".tmp");
...
//    }
    ...
    public static String encryptFile(String aesKey, File inputFile) {
        try {
            ...
            cipher.init(Cipher.ENCRYPT_MODE, originalKey );
            FileInputStream inputStream = new FileInputStream(inputFile);
            FileOutputStream outputStream = new FileOutputStream(new File("src/main/resources/temp/"+ timeStamp + ".encrypted"));
            byte[] buffer = new byte[64];
            int bytesRead;
            while ((bytesRead = inputStream.read(buffer)) != -1) {
                byte[] output = cipher.update(buffer, 0, bytesRead);
    ...
    public static String decryptFile(String aesKey, File inputFile, String fileType) {
        try {
            ...
            cipher.init(Cipher.DECRYPT_MODE, originalKey );
            FileInputStream inputStream = new FileInputStream(inputFile);
            FileOutputStream outputStream = new FileOutputStream(new File("src/main/resources/temp/"+ timeStamp + ".decrypted."+ fileType));
            byte[] buffer = new byte[64];
            int bytesRead;
            while ((bytesRead = inputStream.read(buffer)) != -1) {
                byte[] output = cipher.update(buffer, 0, bytesRead);
