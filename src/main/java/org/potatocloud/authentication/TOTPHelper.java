...
public class TOTPHelper {
	...
	public static InputStream generateQRasFile(String issuer, String label, String secret, Integer digit,
			Integer period) {
		QrData data = new QrData.Builder().label(label).secret(secret).issuer(issuer).algorithm(HashingAlgorithm.SHA1) // More
																														// on
																														// this
																														// below
				.digits(digit).period(period).build();
	...
	public static String generateQRasBase64(String issuer, String label, String secret, Integer digit, Integer period) {
		try {
			QrData data = new QrData.Builder().label(label).secret(secret).issuer(issuer)
